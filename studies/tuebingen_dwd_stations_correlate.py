...
def parse_and_filter_invalid_dates(data: pl.LazyFrame):
    for datecol in ["Beginn", "Ende"]:
        ...
        data = data.with_columns(pl.col(datecol).str.to_date(format="%d.%m.%Y"))
    return data
...
def better_columns(data: pl.LazyFrame):
    return data.with_columns(
        ...
        pl.struct(
            [pl.col("Breite").alias("latitude"), pl.col("Länge").alias("longitude")]
        ).alias("coord"),
    )
...
def add_distance_tüb(data: pl.LazyFrame):
    tübingen_coord = (
        data.filter(pl.col("Stationsname") == "Tübingen").first().collect()["coord"][0]
    )
    ...
    return data.with_columns(
        (
            pl.col("coord")
            .map_elements(
                lambda coord: gpd.distance(
                    (coord["latitude"], coord["longitude"]),
                    (tübingen_coord["latitude"], tübingen_coord["longitude"]),
                ).kilometers,
...
def filter_acceptable_stations(data: pl.LazyFrame):
    data = data.filter(
        pl.col("Beginn") < (dt.date.today() - dt.timedelta(days=1 * 12 * 30))
    )
...
def main():
    ...
    data = add_distance_tüb(better_columns(parse_and_filter_invalid_dates(data)))
    data = filter_acceptable_stations(data)
    data = groupby_location(data)
    data = data.sort(pl.col("distance_tübingen_km"))

    dump_as_json_to(
        convert_to_obj(data), path="./project/data/dwd/aggregated_station_info.json"
    )
